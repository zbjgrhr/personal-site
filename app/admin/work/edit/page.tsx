...
export default function EditWorkPostPage() {
    ...
    fetch("/api/work/posts")
      ...
      .then((data: { posts: Post[] }) => {
        ...
        if (p) {
          ...
          setAudioUrls(p.audioUrls ?? []);
          setPdfUrls(p.pdfUrls ?? []);
          setZipUrls(p.zipUrls ?? []);
        }
      })
      .catch(() => setPost(null))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/work/posts?id=${encodeURIComponent(id)}`, {
        method: "PUT",
  ...
  if (!id || !post) {
    return (
      <div className="space-y-4">
        ...
        <Link href="/admin/work" className="text-sm underline hover:no-underline">
          ← Work posts
        </Link>
      </div>
    );
