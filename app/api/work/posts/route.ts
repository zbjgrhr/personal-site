...
export async function POST(request: NextRequest) {
  ...
  try {
    const savedPost = await mutateKvCollection<WorkPost, WorkPost>(
      KEY,
      parsePosts,
      (posts) => {
        posts.unshift(post);
...
export async function PUT(request: NextRequest) {
  ...
  const slug =
    typeof body.slug === "string" && body.slug.trim()
      ...
      : title.replace(/\s+/g, "-").toLowerCase().replace(/[^a-z0-9-]/g, "");
  try {
    const updatedPost = await mutateKvCollection<WorkPost, WorkPost | null>(
      KEY,
      parsePosts,
      (posts) => {
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    await mutateKvCollection<WorkPost, boolean>(KEY, parsePosts, (posts) => ({
