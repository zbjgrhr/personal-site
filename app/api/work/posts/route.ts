...
function isWorkPost(p: unknown): p is WorkPost {
  return (
    !!p &&
    typeof p === "object" &&
...
export async function PUT(request: NextRequest) {
  ...
  try {
    const post = await mutatePostStore(KEY, (posts) => {
      const idx = posts.findIndex((p) => isWorkPost(p) && p.id === id);
      if (idx === -1) {
        throw new PostNotFoundError(id);
      }

      const updated = {
        ...(posts[idx] as WorkPost),
        title,
        content,
        slug,
        imageUrls,
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    await mutatePostStore(KEY, (posts) => {
      const filtered = posts.filter((p) => !(isWorkPost(p) && p.id === id));
