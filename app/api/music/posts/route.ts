...
import { mutateKvCollection } from "@/lib/kvCollectionStore";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const savedPost = await mutateKvCollection<MusicPost, MusicPost>(
      KEY,
      parsePosts,
      (posts) => {
        posts.unshift(post);
...
export async function PUT(request: NextRequest) {
  ...
  const slug =
    typeof body.slug === "string" && body.slug.trim()
      ...
      : title.replace(/\s+/g, "-").toLowerCase().replace(/[^a-z0-9-]/g, "");
  try {
    const updatedPost = await mutateKvCollection<MusicPost, MusicPost | null>(
      KEY,
      parsePosts,
      (posts) => {
        const idx = posts.findIndex((p) => p.id === id);
        if (idx === -1) {
          return { posts, result: null, save: false };
        }
        posts[idx] = {
          ...posts[idx],
          title,
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    await mutateKvCollection<MusicPost, boolean>(KEY, parsePosts, (posts) => ({
