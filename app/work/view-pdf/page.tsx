...
import { findAllowedPdfUrl } from "@/lib/workPdfAccess";

const KEY = "work:posts";

async function getAllowedPdfUrl(url: string | null | undefined): Promise<string | null> {
  try {
    const data = await kv.get<unknown>(KEY);
    return findAllowedPdfUrl(url, data ?? []);
  } catch {
    return null;
  }
}
...
export default async function ViewPdfPage({
  ...
  const allowedUrl = await getAllowedPdfUrl(url);
